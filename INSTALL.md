...
```bash
sudo sed -i 's|GRUB_CMDLINE_LINUX_DEFAULT=.*|GRUB_CMDLINE_LINUX_DEFAULT="quiet splash pci=realloc,assign-busses,use_crs,hp_mmio_window,hpmmioprefsize=32G,hpmemsize=256M pcie_aspm=off ibt=off"|' /etc/default/grub
sudo sed -i 's|GRUB_CMDLINE_LINUX=.*|GRUB_CMDLINE_LINUX="pm_async=off intel_iommu=on iommu=pt"|' /etc/default/grub
sudo update-grub
```

Parámetros explicados:
- `pci=realloc,assign-busses,use_crs,hp_mmio_window` — Combo definitivo para MacBooks T2. Permite usar ventanas ACPI de Apple y obliga a Thunderbolt a usar la zona de memoria de 64-bits (evitando el límite de 256MB).
